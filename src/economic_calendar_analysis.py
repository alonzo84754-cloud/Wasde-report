...
def analyze_economic_failure(events_df):
    ...
    for _, event in events_df.iterrows():
        ...
        for _, m in mapping.iterrows():
            ...
            # IMPORTANT: Do NOT use 'previous' as forecast fallback
            # 'previous' is last period's ACTUAL value, not a forecast
            # Only process events that have BOTH actual AND forecast values
            if pd.isna(actual) or pd.isna(forecast):
                print(f"  Skipping {event_name} - missing {'actual' if pd.isna(actual) else 'forecast'}")
                continue
